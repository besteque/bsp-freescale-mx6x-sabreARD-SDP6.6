...
static int sdmmc_create_ability( const char* const name, const bool nonroot )
{
	int	 ret;

	if( ( ret = procmgr_ability_create(name, PROCMGR_ADN_ROOT | ( ( nonroot ) ? PROCMGR_ADN_NONROOT : 0U ) ) ) < 0 ) {
		cam_slogf( _SLOGC_SIM_MMC, _SLOG_ERROR, 1, 1, "%s (%s): %s", __FUNCTION__, name, strerror( -ret ) );
		ret = -1;
	}
	return( ret );
}

static int sdmmc_check_ability( resmgr_context_t *ctp, const int ioflag, struct _client_info **info_p, uint32_t abil_id )
{
	struct  _client_able	ability;
	struct  _client_able	*abilp;
	int						ability_count;
	int						error;

	ability_count		= 0;
	ability.flags		= 0;
	ability.ability		= abil_id;
	ability.range_hi	= INT_MAX;
	ability.range_lo	= 0;

	abilp = ((_Uint32t)abil_id == -1) ? NULL: &ability;
	ability_count = (NULL == abilp) ? 0 : 1;

	if( ( error = iofunc_client_info_able( ctp, 0, info_p, IOFUNC_CLIENTINFO_GETGROUPS, abilp, ability_count ) ) != EOK ) {
		return( error );
	}

	// Operation is allowed if the client either has the ability, or is root.
	// The ability is automatically granted to all root processes, so the
	// lack of an ability means either non-root (and not allowed), or the
	// root process explicitly dropped this ability.
	error = EACCES;
	if( info_p && !( (*info_p)->flags & _NTO_CI_UNABLE ) ) {
		error = EOK;
	}
	return( error );
}
...
int main( int argc, char *argv[] )
{
	...
	sdmmc_ctrl.abilities.assd = sdmmc_create_ability( SDMMC_ABILITY_ASSD, false );
...
int sdmmc_partition_enum( SIM_HBA *hba )
{
	...
	uint32_t			*cid;
	uint32_t			mid;
	uint64_t			m_boot_size_ext;
	
	ext				= (SIM_SDMMC_EXT *)hba->ext;
	ecsd			= sdio_get_raw_ecsd( ext->device );
	cid			= sdio_get_raw_cid( ext->device );
	mid			= sdio_extract_bits(cid, 128, 120, 8);
	...
	for( pidx = 0; pidx < MMC_BOOT_PART_MAX; pidx++, part++ ) {
		...
		//BOOT_SIZE_EXT: vendor specific register for Micron to increase boot partition size.
		if ( (mid == MID_MMC_MICRON) && (ecsd[ECSD_PARTITION_SETTING] & ECSD_PS_CMP) && (m_boot_size_ext = (ecsd[ECSD_MICRON_BOOT_SIZE_EXT2] << 16) | (ecsd[ECSD_MICRON_BOOT_SIZE_EXT1] << 8) | ecsd[ECSD_MICRON_BOOT_SIZE_EXT0]) ) {
			part->nlba = (uint64_t)m_boot_size_ext * MMC_BOOT_SIZE_MULT / 512;
		} else {
...
int sdmmc_devctl( SIM_HBA *hba, CCB_DEVCTL *ccb )
{
	...
	switch( ccb->cam_devctl_dcmd ) {
		...
		case DCMD_SDMMC_ASSD_STATUS:
			if( sdmmc_check_ability( ccb->cam_devctl_ctp, ccb->cam_devctl_ioflag, &info_p, sdmmc_ctrl.abilities.assd ) == EOK ) {
	...
	if( info_p ) {
		iofunc_client_info_ext_free( &info_p );
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devb/sdmmc/sim_sdmmc.c $ $Rev: 813585 $")
#endif
