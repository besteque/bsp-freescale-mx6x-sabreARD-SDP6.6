...
int sdio_soc_config( sdio_hc_t *hc, int idx, unsigned offset )
{
	...
	while( ( offset = hwi_next_tag( offset, 1 ) ) != HWI_NULL_OFF ) {
		...
		if( !strcmp( name, HWI_TAG_NAME_busattr ) ) {
			...
			strncpy( cfg->name, __hwi_find_string( tag->dll.name ), sizeof( cfg->name ) );
		}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devb/sdmmc/sdiodi/soc.c $ $Rev: 712278 $")
#endif
