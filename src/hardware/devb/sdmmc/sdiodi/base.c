...
//		if( ( status = pthread_cond_wait( &hc->cond, &hc->mutex ) ) != EOK ) {
			sdio_slogf( _SLOGC_SDIODI, _SLOG_ERROR, hc->cfg.verbosity, 0, "%s: TIMEOUT (errno %d) CMD %d, flgs 0x%x, arg 0x%x, blks %d, blksz %d", __FUNCTION__, status, cmd->opcode, cmd->flags, cmd->arg, cmd->blks, cmd->blksz );
			break;
		}
	}
	...
	while( options && *options != '\0' && status == EOK ) {
		...
		switch( opt ) {
			...
			case 6:			// addr
				...
				cfg->base_addr[cfg->base_addrs] = strtoull( argstr[0], NULL, 0 );
				if( argstr[1] != NULL )
					cfg->base_addr_size[cfg->base_addrs] = strtoull( argstr[1], NULL, 0 );
				cfg->base_addrs++;
				break;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devb/sdmmc/sdiodi/base.c $ $Rev: 813585 $")
#endif
