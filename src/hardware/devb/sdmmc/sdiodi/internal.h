...
static inline int sdio_sg_nxt( sdio_hc_t *hc, uint8_t **addr, int *len, int blksz )
{
	sdio_wspc_t	*wspc;

	wspc = &hc->wspc;

	if( wspc->nsg == 1 && !wspc->sgc ) {		// sge's complete
		return( SDIO_TRUE );
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devb/sdmmc/sdiodi/internal.h $ $Rev: 813585 $")
#endif
