...
struct _scsi_req_block {
	...
	DEVOBJ			*dobj;
	ioreq_t			*ioreq;
	int    			nblks;          // remaining blocks in ioreq
	int    			nblks_xfer;     // blocks in-transfer
	mdl_t  			*mdl;           // remaining part of ioreq's MDL
	int    			nmdl;           // remaining entries in ioreq's MDL
	int    			nmdl_xfer;      // MDL entries in-transfer
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devb/include/ntocam.h $ $Rev: 789707 $")
#endif
