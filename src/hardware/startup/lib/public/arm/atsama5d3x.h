...
#define DMAC_DSCR(ch_num)	(0x03C+(ch_num)*(0x28)+(0x8))	// DMAC Channel Descriptor Address Register
    #define DMAC_DSCR_AHB_IF(_i_)   (_i_)
#define DMAC_CTRLA(ch_num)	(0x03C+(ch_num)*(0x28)+(0xC))	// DMAC Channel Control A Register
    #define DMAC_CTRLA_DST_BYTE     (0 << 28)       // Transfer Width for the Destination 8-bit
    #define DMAC_CTRLA_DST_HWORD    (1 << 28)       // Transfer Width for the Destination 16-bit
    #define DMAC_CTRLA_DST_WORD     (2 << 28)       // Transfer Width for the Destination 32-bit
    #define DMAC_CTRLA_DST_DWORD    (3 << 28)       // Transfer Width for the Destination 64-bit
    #define DMAC_CTRLA_SRC_BYTE     (0 << 24)       // Transfer Width for the Source 8-bit
    #define DMAC_CTRLA_SRC_HWORD    (1 << 24)       // Transfer Width for the Source 16-bit
    #define DMAC_CTRLA_SRC_WORD     (2 << 24)       // Transfer Width for the Source 32-bit
    #define DMAC_CTRLA_SRC_DWORD    (3 << 24)       // Transfer Width for the Source 64-bit
    #define DMAC_CTRLA_DCSIZE_1     (0 << 20)       // Destination Chunk Transfer Size, 1 data
    #define DMAC_CTRLA_DCSIZE_4     (1 << 20)       // Destination Chunk Transfer Size, 4 data
    #define DMAC_CTRLA_DCSIZE_8     (2 << 20)       // Destination Chunk Transfer Size, 8 data
    #define DMAC_CTRLA_DCSIZE_16    (3 << 20)       // Destination Chunk Transfer Size, 16 data
    #define DMAC_CTRLA_SCSIZE_1     (0 << 16)       // Destination Chunk Transfer Size, 1 data
    #define DMAC_CTRLA_SCSIZE_4     (1 << 16)       // Destination Chunk Transfer Size, 4 data
    #define DMAC_CTRLA_SCSIZE_8     (2 << 16)       // Destination Chunk Transfer Size, 8 data
    #define DMAC_CTRLA_SCSIZE_16    (3 << 16)       // Destination Chunk Transfer Size, 16 data
    #define DMAC_CTRLA_BTSIZE(_s_)  ((_s_) & 0xFFFF)// Buffer Transfer Size
#define DMAC_CTRLB(ch_num)	(0x03C+(ch_num)*(0x28)+(0x10))	// DMAC Channel Control B Register
    #define DMAC_CTRLB_SIF(_i_)     ((_i_) << 0)    // Source Interface
    #define DMAC_CTRLB_DIF(_i_)     ((_i_) << 4)    // Destination Interface
    #define DMAC_CTRLB_SRC_PIP      (1 << 8)        // Source Picture-in-Picture Mode
    #define DMAC_CTRLB_DST_PIP      (1 << 12)       // Destination Picture-in-Picture Mode
    #define DMAC_CTRLB_SRC_DSCR_DIS (1 << 16)       // Buffer Descriptor Fetch operation is disabled for the source
    #define DMAC_CTRLB_DST_DSCR_DIS (1 << 20)       // Buffer Descriptor Fetch operation is disabled for the destination
    #define DMAC_CTRLB_FCM2M        (0 << 21)       // Memory-to-Memory Transfer DMAC is flow controller
    #define DMAC_CTRLB_FCM2P        (1 << 21)       // Memory-to-Peripheral Transfer DMAC is flow controller
    #define DMAC_CTRLB_FCP2M        (2 << 21)       // Peripheral-to-Memory Transfer DMAC is flow controller
    #define DMAC_CTRLB_FCP2P        (3 << 21)       // Peripheral-to-Peripheral Transfer DMAC is flow controller
    #define DMAC_CTRLB_SRC_INCR     (0 << 24)       // The source address is incremented
    #define DMAC_CTRLB_SRC_DECR     (1 << 24)       // The source address is decremented
    #define DMAC_CTRLB_SRC_FIXD     (2 << 24)       // The source address is fixed
    #define DMAC_CTRLB_DST_INCR     (0 << 28)       // The destination address is incremented
    #define DMAC_CTRLB_DST_DECR     (1 << 28)       // The destination address is decremented
    #define DMAC_CTRLB_DST_FIXD     (2 << 28)       // The destination address is fixed
    #define DMAC_CTRLB_IEN          (1 << 30)       // Interrupt Enable Not
    #define DMAC_CTRLB_AUTO         (1 << 31)       // Automatic Multiple Buffer Transfer
#define DMAC_CFG(ch_num)	(0x03C+(ch_num)*(0x28)+(0x14))	// DMAC Channel Configuration Register
    #define DMAC_CFG_SRC_PER(_p_)   ((((_p_) & 0x0F) << 0) | (((_p_) & 0x30) <<  6))    // Source with Peripheral identifier
    #define DMAC_CFG_DST_PER(_p_)   ((((_p_) & 0x0F) << 4) | (((_p_) & 0x30) << 10))    // Destination with Peripheral identifier
    #define DMAC_CFG_SRC_REP        (1 << 8)        // Source Reloaded from Previous
    #define DMAC_CFG_SRC_H2SEL      (1 << 9)        // Hardware Selection for the Source
    #define DMAC_CFG_DST_REP        (1 << 12)       // Destination Reloaded from Previous
    #define DMAC_CFG_DST_H2SEL      (1 << 13)       // Hardware Selection for the Destination
    #define DMAC_CFG_SOD            (1 << 16)       // Stop On Done
    #define DMAC_CFG_LOCK_IF        (1 << 20)       // Interface Lock capability is enabled
    #define DMAC_CFG_LOCK_B         (1 << 21)       // AHB Bus Locking capability is enabled
    #define DMAC_CFG_LOCK_IF_L      (1 << 22)       // Master Interface Arbiter Lock
    #define DMAC_CFG_AHB_PROT_MASK  (7 << 24)       // AHB Protection Mask
    #define DMAC_CFG_AHB_PROT_PRIV  (1 << 24)       // Privileged Access
    #define DMAC_CFG_AHB_PROT_BUFF  (1 << 25)       // Bufferable
    #define DMAC_CFG_AHB_PROT_CACHE (1 << 26)       // Cacheable
    #define DMAC_CFG_AHB_FIFOCFG_MASK   (3 << 28)   // FIFO Configuration
    #define DMAC_CFG_AHB_FIFOCFG_ALAP   (0 << 28)   // The largest defined length AHB burst is performed on the destination AHB interface
    #define DMAC_CFG_AHB_FIFOCFG_HALF   (1 << 28)   // When half FIFO size is available/filled, a source/destination request is serviced
    #define DMAC_CFG_AHB_FIFOCFG_ASAP   (2 << 28)   // When there is enough space/data available to perform a single AHB access, then the request is serviced.
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/startup/lib/public/arm/atsama5d3x.h $ $Rev: 813406 $")
#endif
