...
int imx_i2c_options(imx_dev_t *dev, int argc, char *argv[])
{
    ...
    while (!done) {
        ...
        switch (c) {
            ...
            case 't':
                dev->slave_resp_timeout_slack = strtoul(optarg, &optarg, NULL);
                break;

            case 'y':
                dev->bus_busy_timeout = strtoul(optarg, &optarg, NULL);
                break;

            case 'h':
                dev->intr_priority = strtoul(optarg, &optarg, NULL);
                break;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/i2c/imx/options.c $ $Rev: 770939 $")
#endif
