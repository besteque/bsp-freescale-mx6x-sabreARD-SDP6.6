...
void ser_stty(DEV_MX1 *dev)
{
   ...
   /* Use rfdiv value which produces the most accurate baud rate */
    ...
    if(dev->tty.verbose >= 5){
        slogf(_SLOG_SETCODE(_SLOGC_CHAR, 0), _SLOG_INFO, "requested baud rate: %ubps, (approximate) actual baud rate = %u, rfdiv: %u, UBMR+1: %d, UBIR+1: %d", dev->tty.baud, best_baud, rfdiv, 9999+1, bir+1);
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devc/sermx1/tto.c $ $Rev: 814269 $")
#endif
