...
static const struct sigevent *spi_intr(void *area, int id)
{
	...
	if (tmp & CSPI_STATREG_TC) {
		...
		if(count > MX51_CSPI_FIFO_RXMARK) {
	...
	if (dev->burst) {
		 ...
		 */
		count = MX51_CSPI_FIFO_SIZE - (in32(base + MX51_CSPI_TESTREG) & CSPI_TESTREG_TXCNT_MASK);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/spi/mx51ecspi/intr.c $ $Rev: 811210 $")
#endif
